...
@section('content')
    <div class="mb-8">
        <h1 class="text-2xl font-semibold tracking-tight text-zinc-900">Edit language</h1>
        <p class="mt-1 text-sm text-zinc-500">{{ $language->code }}</p>
    </div>

    <div class="max-w-xl rounded-2xl border border-zinc-200/80 bg-white p-8 shadow-sm">
        <form action="{{ route('dashboard.languages.update', $language) }}" method="post" class="space-y-5">
            @csrf
            @method('PUT')
            <div>
            ...
            <div>
                ...
                <input id="sort_order" name="sort_order" type="number" min="0" value="{{ old('sort_order', $language->sort_order) }}"
                    class="block w-full rounded-xl border border-zinc-200 px-4 py-2.5 text-sm focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/20">
            </div>
            <div class="flex items-center gap-2">
                <input id="is_active" name="is_active" type="checkbox" value="1" class="rounded border-zinc-300 text-indigo-600 focus:ring-indigo-500/30" @checked(old('is_active', $language->is_active))>
                <label for="is_active" class="text-sm text-zinc-700">Active on site</label>
            </div>
            <div class="flex items-center gap-2">
                <input id="is_default" name="is_default" type="checkbox" value="1" class="rounded border-zinc-300 text-indigo-600 focus:ring-indigo-500/30" @checked(old('is_default', $language->is_default))>
                <label for="is_default" class="text-sm text-zinc-700">Default language</label>
            </div>
            <div class="flex gap-3 pt-2">
                <button type="submit" class="rounded-xl bg-zinc-900 px-4 py-2.5 text-sm font-semibold text-white hover:bg-zinc-800">Update</button>
