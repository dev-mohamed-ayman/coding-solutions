<footer class="relative z-10 border-t border-white/5 bg-[#050510]/80 backdrop-blur-md">
    <div
        ...
        <nav class="flex flex-wrap justify-center gap-6" aria-label="Footer">
            <a class="nav-link text-slate-400 hover:text-primary transition-colors" href="{{ request()->routeIs('home') ? '#services' : route('home').'#services' }}">{{ site_t('nav.services') }}</a>
            <a class="nav-link text-slate-400 hover:text-primary transition-colors" href="{{ request()->routeIs('home') ? '#portfolio' : route('home').'#portfolio' }}">{{ site_t('nav.portfolio') }}</a>
            <a class="nav-link text-slate-400 hover:text-primary transition-colors" href="{{ request()->routeIs('home') ? '#stats' : route('home').'#stats' }}">{{ site_t('nav.about') }}</a>
