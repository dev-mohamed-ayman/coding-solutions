...
<header class="fixed top-0 w-full z-50 header-glass" id="header">
    <div class="flex justify-between items-center px-8 py-5 max-w-7xl mx-auto gap-4">
        ...
        <!-- Mobile Menu Button -->
        <button class="md:hidden text-white shrink-0 relative z-50 p-2" id="mobileMenuBtn" type="button" aria-label="Toggle menu">
            <span class="material-symbols-outlined text-3xl" id="menuIcon">menu</span>
        </button>
    </div>

    <!-- Mobile Menu Overlay -->
    <div id="mobileMenu" class="fixed inset-0 bg-[#050510]/98 backdrop-blur-2xl z-40 hidden flex-col transition-all duration-500 ease-in-out opacity-0 translate-x-full">
        <div class="flex flex-col h-full pt-32 px-8 pb-10">
            ...
            @if (isset($activeLanguages) && $activeLanguages->count() > 1)
                <div class="mt-auto border-t border-white/10 pt-8">
                    <p class="text-xs uppercase tracking-[0.2em] text-slate-500 font-bold mb-4">{{ site_t('common.language') ?? 'Language' }}</p>
